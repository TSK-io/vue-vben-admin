...
def test_input_role_can_lookup_elder_phone(client: TestClient) -> None:
    headers = auth_headers(client, "input_demo", "111")

    me_response = client.get("/api/v1/auth/me", headers=headers)
    assert me_response.status_code == 200
    assert "input" in me_response.json()["data"]["roles"]

    lookup_response = client.get(
        "/api/v1/phone-directory/lookup",
        headers=headers,
        params={"phone": "13800001001", "role": "elder"},
    )
    assert lookup_response.status_code == 200
    data = lookup_response.json()["data"]
    assert data["user_id"] == "u-elder-001"
